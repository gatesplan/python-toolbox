...
SpotWallet MUST manage currency balances (quote currencies like USD, KRW) through deposit and withdrawal operations and record all operations in ledgers.
...
assert df.iloc[1]["value_change"] == -3000.0
assert df.iloc[1]["value"] == 7000.0
```
...
SpotWallet MUST process BUY trades by deducting quote currency and adding to PairStack, recording both trade and fee in ledger.
...
df = ledger.to_dataframe()
assert len(df) == 1
assert df.iloc[0]["event"] == "buy"
assert df.iloc[0]["asset_change"] == 1.0
assert df.iloc[0]["value_change"] == -50000.0
```

#### Scenario: 수수료 포함 BUY 거래
```python
wallet = SpotWallet()
wallet.deposit_currency(symbol="USD", amount=100000.0)
...
trade = SpotTrade(
    ...
    pair=Pair(Token("BTC", 1.0), Token("USD", 50000.0)),
    timestamp=1234567890,
    fee=Token("USD", 50.0)  # 0.1% fee
)

wallet.process_trade(trade)

# Ledger 확인: BUY + FEE 두 개 entry
ledger = wallet.get_ledger("BTC-USD")
df = ledger.to_dataframe()
assert len(df) == 2
assert df.iloc[0]["event"] == "buy"
assert df.iloc[1]["event"] == "fee"
assert df.iloc[1]["value_change"] == -50.0
```
...
# Ledger 확인: 두 개의 BUY entry
ledger = wallet.get_ledger("BTC-USD")
df = ledger.to_dataframe()
assert len(df) == 2
assert df.iloc[0]["asset_change"] == 1.0
assert df.iloc[1]["asset_change"] == 0.5
assert df.iloc[1]["asset"] == 1.5  # 누적
```

### Requirement: SELL 거래 처리
SpotWallet MUST process SELL trades by removing assets from PairStack (FIFO) and adding quote currency, recording both trade and fee in ledger.
...
df = ledger.to_dataframe()
assert len(df) == 2
assert df.iloc[1]["event"] == "sell"
assert df.iloc[1]["asset_change"] == -0.6
assert df.iloc[1]["value_change"] == 33000.0
assert df.iloc[1]["asset"] == 0.4
```
...
#### Scenario: Ledger 조회 - 화폐 계정
```python
wallet = SpotWallet()
wallet.deposit_currency(symbol="USDT", amount=10000.0)

ledger = wallet.get_ledger("USDT")
assert ledger is not None
assert ledger.account == "USDT"
```

#### Scenario: Ledger 조회 - 자산 계정
```python
wallet = SpotWallet()
wallet.deposit_currency(symbol="USD", amount=100000.0)
