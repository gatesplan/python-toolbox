...
class SpotWallet:
    ...
    def get_pair_stack(self, ticker: Union[str, Symbol]) -> Optional[PairStack]:
        """특정 거래쌍의 PairStack 조회. ticker는 문자열 또는 Symbol 객체."""
        # Symbol 객체면 dash 형식으로 변환
        ticker_key = ticker.to_dash() if isinstance(ticker, Symbol) else ticker

        if ticker_key not in self._pair_stacks:
            return None
        stack = self._pair_stacks[ticker_key]
        if stack.is_empty():
            return None
        return stack

    def get_ledger(self, ticker: Union[str, Symbol]) -> Optional[SpotLedger]:
        """특정 거래쌍의 장부 조회. ticker는 문자열 또는 Symbol 객체."""
        # Symbol 객체면 dash 형식으로 변환
        ticker_key = ticker.to_dash() if isinstance(ticker, Symbol) else ticker
        return self._ledgers.get(ticker_key)
